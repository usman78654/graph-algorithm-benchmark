...
def compute_diameter(graph, nodes, limit=500):
    diameter = 0
    memo = {}
    ...
    with open("diameter_trace.txt", "w", encoding="utf-8") as trace_file:
        ...
        for idx, node in enumerate(nodes_to_process):
            if idx % 50 == 0:
                ...
                print(log_line)
            trace_file.write(f"Processing node {idx + 1}: {node}\n")
...
if __name__ == "__main__":
    ...
    print("Loading graph for diameter calculation...")
    ...
    with open("diameter_executionTime.txt", "w", encoding="utf-8") as f:
